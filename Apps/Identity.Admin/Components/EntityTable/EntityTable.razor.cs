    ...
    where TRequest : new()
{
    ...
    { 
               ...
               is { } result1)
       {
           if (result1.TotalRecords <= 0)
               Toast.Add(result1.Message ?? string.Empty, Severity.Error);
           else
               ...
               is { } result)
       {
           if (result.TotalRecords <= 0)
               Toast.Add(result.Message?? String.Empty, Severity.Error);
           else
    ...
    private async Task InvokeImportModal()
    {
        ...
        var dialog = await DialogService.ShowAsync<ImportModal>("Import", parameters, options);
    ...
    private async Task EditSelectedAsync(HashSet<TEntity> selectedItems)
    {
        ...
        await DialogService.ShowAsync<DialogNotification>("Warning", parameters, options);
    }
    ...
    private async Task DeleteSelectedAsync(HashSet<TEntity> selectedItems)
    {
        ...
        else
        {
            ...
            var dialog = await DialogService.ShowAsync<DialogComfirmation>("Delete", parameters, options);
